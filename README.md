# elixir_ale_rpi - Elixir Actor Library for Embedded - Raspberry Edition

[![Build Status](https://travis-ci.org/Hermanverschooten/elixir_ale_rpi.svg)](https://travis-ci.org/Hermanverschooten/elixir_ale_rpi)
[![Hex version](https://img.shields.io/hexpm/v/elixir_ale_rpi.svg "Hex version")](https://hex.pm/packages/elixir_ale_rpi)

This is a special edition of the fantastic elixir_ale library maintained by Frank Hunleth.
It contains some additions that are only available on Raspberry Pi.
Most of the documentation refers to elixir_ale, this has been kept intentionally.
I will try to follow changes to the original project and apply them here if relevant.
...
`mix.exs` like this:
    ...
    defp deps do
      [{:elixir_ale_rpi, "~> 0.5.8"}]
    end

If you just want to try it out, you can do the following:

    git clone https://github.com/Hermanverschooten/elixir_ale_rpi.git
    cd elixir_ale_rpi
    mix compile
    iex -S mix
